...
### 9.21 進捗（本セッション）: `num` 上の算術 -- と、`NUMPAIR` が要らないことの決定

`num_rec` で `num_add`/`num_mul` を定義し、節方程式 4 本を導出した
（仮説 0、公理は 1 本のまま）。

    num_add(zero, n)   = n
    num_add(suc m, n)  = suc(num_add(m, n))
    num_mul(zero, n)   = zero
    num_mul(suc m, n)  = num_add(num_mul(m, n), n)

**これは飾りではなく、フェーズ 3 の critical path 上にある。同時に
`NUMPAIR` は要らないという設計判断を確定した。**

HOL Light の `ind-types.ml` が `NUMPAIR`（= `2^x * (2y+1) - 1`）を必要と
するのは、`recspace` の子ノードを `num -> recspace` という**任意濃度の族**
として扱うからである。その単射性の証明には 2 進分解の一意性、つまり
`arith.ml` 相当の数論が要る。

しかしこのシステムの datatype は `check_spec` により

* 相互再帰なし
* 入れ子なし
* コンストラクタのフィールドは**固定長の有限リスト**

に制限されている。したがって**分岐数には型ごとに定数上界 `k` がある**。
すると木のノードは幅優先で番号付けでき、`n` 番の子は
`k*n+1 .. k*n+k` になる。必要なのは `+` と `*` だけで、対関数も
その一意分解も要らない。制限された datatype クラスが本家より単純な
構成を許す、というのは §9.2 フェーズ 3 の記述が最初から予告していた点で、
ここで具体的に何が落ちるかが確定した。

残りのフェーズ 3 は、この番号付けの上に

1. `num -> (Tag × NonRecFields) + unit` の部分集合として木を書く
   （`+`/`×` は `algebra.rhm` の導出済み `sum`/`prod`）、
2. 整合性述語（根が存在し、子の有無がタグのアリティと一致し、有限）で
   `new_basic_type_definition` により切り出し、
3. injectivity を番号付けの単射性から、distinctness をタグの相違から、
   induction を切り出した集合の最小性から、cases を構成の全射性から出す、
