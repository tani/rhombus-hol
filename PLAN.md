...
### 重複ゴール検出・書き換えループ検出は「解くべき問題が計測できない」ので見送った

induction pool の撤回（上記）と同じ系統の項目として、(a) waterfall の
重複ゴール検出と (b) `simp_conv` の書き換えループ検出を検討した。今回まず
計測した:

```text
tests/decl_theorem.rhm   3.19 秒
tests/waterfall.rhm      0.53 秒
tests/induct.rhm         0.35 秒
フルスイート            38 秒（1193 テスト）
```

induction pool を試したときの記録にある「decl_theorem が 29 秒」は当時の
状態のもので、現在は 3.2 秒である。waterfall はスイート全体のボトルネック
ではない。

(a) 重複ゴール検出は、固定深度 2 のままなら純粋な枝刈りだが、刈れる量が
上の数字に対して意味を持たない。しかも安全にやるには「同じゴールが**同じ
か少ない残り深度で**再出現したときだけ刈る」必要があり（残り深度が違えば
二度目は違う振る舞いをしうる）、得られる速度に対して条件が細かすぎる。

(b) 書き換えループ検出は速度の話ではなく、`mk_rule` が弾けない非停止規則が
`raco make` を止めるという可用性の問題である。これは実在する危険だが、
**現在のコーパスでは一度も起きていない**（意図的に失敗させるフィクスチャを
含めて、全ファイルが上記の時間で終わる）。`trust.scrbl` は「fuel も timeout
も意図的に置いていない」と明記しており、観測されていない問題のために
簡約器の中心に状態を持ち込むのは、induction pool で学んだのと同じ失敗の
形になる。着手するなら、まず**再現するフィクスチャ**を書いてから。
