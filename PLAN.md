...
| 表層構文 | `surface_space` `surface_notation` `surface_core` `expand` `elab` `driver` `taut` `module_block` | `lang_state` `decl_type` `decl_fun` `decl_theorem` `surface_notation` |
...
- `function` の本体は `match` / `if` / `cond` / 局所 `let` / 変数 / 名前付き適用 /
  Boolean 演算子、`~runtime` と `~logic` を持つ notation、`#true` / `#false` のみ。
...
### 9.26 進捗（本セッション）: `let` の評価義務 -- 論理側が落とす再帰呼出し

第三次レビュー項目 3。`let` は束縛ではなく**代入**なので、initializer は
変数が読まれた箇所にしか残らない。一方で実行側は元の Rhombus を出力する
ので、読まれなくても評価する。結果、次が通っていた:

    function bad(n :: Nat) :: Nat:
      let unused = bad(n)
      zero()

論理側は `!n. bad(n) = zero`（公理 0、停止性の method は `none` -- 再帰が
一つも見えない）。実行側は `bad(zero())` が**発散する**。HOL の関数は全域
なので定理自体は偽ではないが、「コンパイルされたプログラムについての主張」
ではなくなっている。**実測で確認した上で**修正した。

**判定条件は「initializer が再帰する」ではなく「どれかの節がそれを落とした」
にした。** レビューの案（再帰する initializer を一律拒否）だと、

    | succ(k):
        let rest = dbl(k)
        succ(succ(rest))

のような正当な定義まで落ちる（実際に試して落ちた）。正確な条件は
「この body が生む**すべての**節に initializer の値が残っていること」で、
`match` の一部の枝でしか使われない場合は拒否される -- 他の枝はそれを評価
して捨てるからである。

これはまだ**代入後の項**に対する検査である。β 簡約はどう書いても呼出しを
消せるので、恒久的には評価順序を保った core（`Let` の呼出し義務は
...
### 9.28 進捗（本セッション）: 探索の資源上限 -- と、計測を三度誤った話

第三次レビュー項目 5。書き換えには**上限が一つもなかった**。停止せず失敗も
しない規則 -- permutative でなく右辺が小さくもない規則 -- は `simp_conv` を
ループさせ、簡約はモジュールのコンパイル中に走るので、`raco make` は失敗
ではなく**ハング**した。

`search_limits.rhm` を追加した。設計上の要点は二つ。

**予算は共有される。** 入れ子の探索（規則の side condition を discharge し、
それ自身がまた書き換える）は呼び出し側の残量から引く。新しい枠を取ったら、
「上限の隣に無制限のループがある」ことになる。`with_search_limits` は既に
開いている予算があればそれに合流する。

**枯渇はエラーで、`#false` ではない。** 変換の `#false` は「この規則はここに
適用されない」という正常な答えである。資源切れが同じ値を返したら、証明は
探索を打ち切った経路を黙って進み、失敗は全く別の場所に現れる。

課金点は**反復そのもの**（`REPEATC_BOUNDED`）と**waterfall の各訪問**だけ。
`db_conv` の規則候補ループは有限（`h` の下の規則を一巡するだけ）なので課金
しない -- そこで課金すると、一度しか書き換えない裸の `db_conv` 呼び出しにまで
資源スコープを要求することになる。スコープは `BOUNDED_TOP_DEPTH_CONV` が
適用のたびに開く（既にあれば合流）。呼び出し側に義務を課さないので、
「スコープを開き忘れた」という配線バグの類が存在し得ない。

上限は**一つの数**（steps）にした。深さ上限は冗長である: 有界走査は各段で
必ず 1 歩課金してから再帰するので、歩数が深さも縛る。

**計測を三度誤った。** 記録しておく価値がある種類の間違いだった。

1. スイートが 60s から 254s になったのを回帰と読んだ。実際には自分で
   `compiled/` を全消ししていたので、**コールドコンパイル**を測っていた。
2. 個別ファイル（`raco test decl_theorem.rhm`）で切り分けようとしたが、
   テストファイルは `raco make` されていないので**毎回コンパイルから**走り、
   16s のうち大半がコンパイルだった。HEAD でも同じ 16.5s が出て、ようやく
   気づいた。
3. 二分探索の途中の測定（2.58s / 2.83s）は、スコープ未開通のエラーで
   **テストが早期に落ちていた**だけ。`tail -1` で時間しか見ておらず、
   合否を見ていなかった。

正しい手順は「`raco make tests/*.rhm` してからスイート全体を計測し、毎回
合否も一緒に見る」。そうして測った結果、**回帰は最初から存在しなかった**
（63.5s、ベースライン通り）。適用ごとのスコープ開設も 65.7s vs 63.5s で
誤差の範囲であり、当初「6倍」と判断して外そうとしたのは全くの誤り。
安全側（呼び出し側に義務なし）を採った。

回帰は `tests/search_limits.rhm`: cycle 検出、成長規則の停止、通常の書き換えが
影響を受けないこと、そして**入れ子の予算共有**（外側 10 歩のうち 1 歩使って
から内側を回すと、内側は 9 歩で尽きる）。

### 9.29 進捗（本セッション）: `T_lt` の脱公理化 -- 直接子関係の推移閉包

第三次レビュー項目 6。`type` 宣言の経路に残っていた最後の `new_axiom` を
消した。datatype ごとの subterm relation `T_lt` は、これまで
`subterm_spec` が節形式の定義を作り `install_function` がその方程式を
`new_axiom` で立てていた（datatype 1 つあたり 2 本）。

**そもそも再帰的定義である必要がなかった。** 「真部分項」は「直接の子」の
推移閉包であり、「直接の子」は datatype が既に証明している判別子と選択子で
書ける**非再帰**述語である:

```text
T_child x y  <=>  (is_C1 y and (x = C1_f1 y or ...)) or ...
T_lt         =    TC T_child
```

推移閉包は標準の非可述的な定義（`tclosure.rhm`）:

```text
TC R x y <=> !P. (!u v. R u v ==> P u v)
                 and (!u v w. P u v and P v w ==> P u w) ==> P x y
```

定義なので公理はゼロで、帰納原理は `P` を具体化するだけで出る。

**要になるのは `TC_UNFOLD`** -- `TC R x y ==> ?z. R z y and (x = z or TC R x z)`
（連鎖には*最後の一歩*がある）。これが `TC child x (C a1..ak)` を「`C` の
子についての言明」に変えるので、constructor equations が仮定ではなく
定理になる。証明は強めた述語
`Q u v := TC R u v and (?z. R z v and (u = z or TC R u z))` への TC 帰納法。
閉包そのものを連れ回す必要がある -- 推移律の場合分けで「`x` から `y` への
連鎖」自体が要るため。

結果、公理 2 本が定義 2 本と証明に置き換わった。出てくる方程式は
`subterm_clause` が記述する形と同一なので、`prove_wf_t_lt`・rule database・
`stepfn.rhm` は**無変更**。

**台帳の更新**（これが成果の実体）:

* 再帰 datatype 2 つのモジュールの公理数: **8 -> 4**（基礎論理 3 + 無限公理 1）。
  3 つ目を宣言しても増えない。`tests/rec_wired.rhm`。
* `num` を宣言経路で作った場合と導出した場合の比較: **[3, 1] -> [1, 1]**。
  両経路が完全に一致した。`tests/infinity.rhm`。
* 新規回帰 `tests/tclosure.rhm`: 閉包の 3 定理が仮説 0・公理 0、および
  1 再帰フィールド / 2 再帰フィールド + 非再帰コンストラクタ の 2 形状で
  方程式と `wf(T_lt)` が公理 0 で出ること。

途中で踏んだ実装上の罠を 2 つ記録する。どちらも「1 要素の場合」だった。

1. 右結合の選言を i 番目から導入する補助関数で「`i` が最後なら `th` を
   そのまま返す」としていた。正しくは「**選言が 1 要素なら**そのまま」で、
   2 要素の 2 番目には `DISJ2` が要る。
2. 選言の場合分けで、1 要素のとき `ASSUME(ds[0])` を作っていた。`DISJ_CASES`
   が無いので**その仮定を落とす者がいない**。呼び出し側の定理をそのまま
...
| (最終まとめ表 P2) | binding-aware logical names・namespace・複数 theory import | **保守側の安全性は回帰で固定、合流機構は未着手**。互いに拡張関係にない 2 理論の合流は**明示的に拒否**され（`tests/import_theory.rhm` が `"does not extend the one in scope"` を 2 例で固定）、黙って壊れることはない。合流を*可能にする*方は未着手で、以下が理由。`driver.rhm` の `adopt` が兄弟理論（互いに拡張関係にない2理論）を合流できないのは、`Const` の identity が裸の `Symbol` のみで名前空間の概念がないため。安全な合流にはカーネルの項表現そのもの（`Symbol` を定数識別子に使っている全箇所）の変更が要り、独立したセッション規模の課題。詳細は §9.6。 |
...
### 9.22 進捗（本セッション）: フェーズ 3 の要となる補題 -- `node` の単射性

§9.21 で確定した符号化を、紙の上ではなく**実際に証明して**検証した。
`treerep.rhm` を新設し、

    |- !l1 k1 l2 k2. node(l1, k1) = node(l2, k2) <=> l1 = l2 and k1 = k2

を導出した（仮説 0、公理は §9.14 の 1 本のみ）。これが要である理由は
単純で、これが成り立たなければ相異なる値が同じ表現を持ち、切り出した型が
潰れるからだ。datatype のコンストラクタの単射性も、`REP` の最小性から
出す induction も、すべてこの上に乗る。

証明は「選ぶための経路を 2 本作って代入する」だけである:

* 空経路 `\i. 0` -- `then` 枝を通り `inl l1 = inl l2` が残る。
  `algebra.rhm` 導出済みの `inl_inj` で開く。
* 降下経路 `\i. if i = 0 | suc j | q(pre_num i)` -- `else` 枝を通り、
  頭が `pre_num(suc j) = j`、尾が `\i. p(suc i) = q`（外延性）になるよう
  作ってあるので、任意の `j`/`q` について `k1 j q = k2 j q` が残る。
  **任意の経路がこの形で到達できる**ので、単射性は `kids` 関数全体に
  ついて成り立ち、アリティ未満に限定されない。

逆向きは合同性なので、両向きを持つ同値として述べた（切り出しはどちら
向きにも書き換える必要がある）。

**使った補題は §9.21 の表そのままで、算術も対関数も 1 つも要らなかった。**
設計の検証としてはこれが本命である。

付随して `drule.rhm` に **`EXT`**（外延性: `A |- f v = g v` から
`A |- f = g`、`v` は仮説に自由でない）を追加した。標準的な派生規則なのに
無かった。`ABS` で閉じてから eta 簡約する形で、**eta redex になっている
側だけ**畳む -- この符号化が作る `\i. h(suc i) = q` は片側しか redex に
ならないため。

`mk_kids` も追加した。`node` は子を関数 `num -> Rep` で取るが、
コンストラクタの子は固定有限個なので、`n` 個をその関数へ詰め、`n` 以降は
呼び出し側が渡す junk で埋める。**junk のスロットは誰も読まないので、
単射性も読むスロットについてしか主張しない** -- これがアリティ上界を
符号化に一切登場させずに済む理由である。添字リテラル
（`suc_num^m(zero_num)`）の相異性は `suc_injective` と `suc_not_zero` を
交互に剥がして出す（`prove_lit_distinct`）。3 子コンストラクタで
3 スロット全部の単射性を仮説 0 で導出することを回帰で固定した。
...
**cases も導出し、4 本すべてが揃った**（`prove_datatype_cases`）。
述語を選言そのものに取って induction を適用する。各コンストラクタは
手元の引数で自分の選言肢を埋めるだけなので、**case 仮説は一度も使わない**
-- 網羅性は子について何かが成り立つことに依存せず、値の作り方が他に無い
ことだけに依存する、というのがそのまま証明に出る。

**副産物: `EXISTS` の 2 つ目の潜在バグ。** `EXISTS` は「未簡約の
`expect` と照合してから、`BETA_RULE` した含意へ `MP` する」という不整合を
持っていた。witness を入れた本体の**内側に redex が残る**場合に限って
（ここでは `mk_kids` が arity 0 で `\j. junk` になり `node` がそれを適用
するため必ず残る）、照合は通るのに `MP` で落ちる。`th` 側も同じく簡約して
から `MP` するよう直した。§9.16 で直した捕獲バグと同じ「片側だけ簡約」系
である。

**これで自己再帰 datatype の 4 本（injectivity・distinctness・induction・
cases）がすべて導出になった。** リストを実際に切り出して 4 本とも仮説 0、
理論の公理は無限公理 1 本だけ、を回帰で固定してある。残るのは
`DatatypeSpec` からこれらを自動生成する層と `driver.rhm` への配線。

フルスイート 1297/1297。

### 9.24 進捗（本セッション）: 任意の `DatatypeSpec` を切り出せるようにした

`datatype_rec.rhm` を新設。§9.23 までは手で組んだ 1 例だったが、
`check_spec` が認めるクラス（直接自己再帰・非入れ子・非相互再帰）の
**任意の spec** について、ラベル型・タグ定数・junk 部分木・`REP` 述語・型
そのものを spec だけから生成する。

* コンストラクタのラベルは**非再帰フィールドだけ**を担い、どのコンストラクタ
  かでタグ付けされる。これは `datatype_gen.rhm` が非再帰 datatype 用に既に
  作っている sum-of-products そのものなので、機械を**共有**した（複製せず
  `payload_type_of_fields`/`repr_type_from`/`inject_from`/`distinct_repr`/
  `payload_eq_from_inject_eq` を export した）。再帰フィールドは代わりに
  節の子になる。
* タグは**定数として導入する**（ラムダにしない）。ラムダだと `label a` が
  全部 redex になり、途中で簡約された側とされない側が食い違う -- `WFREC`
  が明示的に拒否している罠で、`EXISTS` が黙って踏んでいた罠でもある。
* witness は再帰フィールドを持たないコンストラクタを 1 つ選び、任意の
  payload に特殊化するだけでよい。`@x. true` は任意の型で well-formed な項
  なので、**型の非空性定理は要らない**。全コンストラクタが再帰する spec は
  型が空になるので、切り出さずにエラーにする。

回帰（`tests/datatype_rec.rhm`）は 3 つの形で通してある: 型引数 1 の
リスト、**1 コンストラクタに再帰フィールドが 2 つある**木、型引数なし・
3 コンストラクタ・アリティ混在の式。いずれも型が正しいアリティで生まれ、
2 本の係数定理が仮説 0 で出て、**追加公理は 0**（理論の公理は無限公理
1 本のまま、datatype ごとに増えない）。

残るのは §9.22/9.23 の 4 定理をこの生成物に対して述べ直す層
...
**判別子・選択子・destructor elimination も揃い、完全な `DatatypeThms` が
出るようになった**（`build_recursive_datatype_thms`）。

これらは**新たに証明していない**。`datatype_gen.rhm` の該当部分
（149 行）を `datatype_common.rhm` に**切り出して共有**した。判別子や
選択子の意味は型の作り方に依存せず、injectivity・distinctness・cases
だけに依存する ── 再帰版でもそれらが同じ形で導出できるようになった以上、
2 度証明する理由がない。非再帰版は挙動不変（既存テストで確認）。

差分テストを全部品に拡張し、**判別子・選択子・elim も含めて公理版と
1 本ずつ完全一致**することを確認した:
...
残るのは `driver.rhm` への配線のみ。なお配線しても `T_lt`（部分項関係）の
方程式は当面 `add_subterm_relation` が公理として入れる ── それを導出に
...
さらに `add_type` から **`datatype_axioms` への分岐を削除した**。
`is_nonrecursive_spec` と `is_selfrecursive_spec` は厳密に補集合
（フィールドが自分自身の型であるか、ないか）なので第三の場合が無く、
公理スキーマはもう宣言の経路上に存在しない。`datatype_axioms` 自体は
**導出を突き合わせる差分テストの基準**としてのみ残る（そちらの方が
到達不能なフォールバックであるより良い仕事である）。
...
## 11. `rocq-hol-kernel/` — Rocq 完全形式化の試みと失敗分析（2026-09-08）

### 11.0 目標

`idris-hol-kernel/` が持つもの（十の基本規則それぞれが well-formed な `Thm` を
返すことの証明、理論拡張の単調性、仮説集合の会計的正しさ）に加えて、それが
**明示的に持たないもの**を Rocq (Coq 9.1.1 / mathcomp-ssreflect 2.5.0) で
`rocq-hol-kernel/` として作る計画だった：集合論的な denotational semantics
（`Frame`/`interpType`/`denote`）と、(a) 十規則それぞれが意味論的妥当性
（`Valid`）を保存すること、(b) `new_axiom` を除く 4 つの理論拡張原理
（`new_type` / `new_constant` / `new_basic_definition` /
`new_basic_type_definition`）が保存的（conservative: 拡張前の理論の任意の
モデルが拡張後の理論のモデルへ延長できる）であることの証明。`new_axiom`
は意図的な抜け穴として最初から対象外（保存性の証明を要求しない）。

### 11.1 最終的に達成できたこと（2026-09-08 セッション終了時点）

`src/Names.v`（89 行）・`src/HType.v`（368 行）・`src/Term.v`（445 行）・
`src/Kernel.v`（2181 行、`rhombus-hol-kernel` の逐語訳 + 十規則の
well-formedness 保存 + 4 つの拡張原理の構文的正しさ）・`src/Semantics.v`
（802 行、`Frame`/`interpType`/`denote`/`ModelsTheory`）・`src/Soundness.v`
（3241 行）・`src/PrintAssumptions.v`（50 行）は全てゼロエラー・`Admitted`
ゼロで `make` によりコンパイル済み（`grep -rn "^Admitted\." src/*.v` は
空）。十規則全部（`reflValid`/`assumeValid`/`transValid`/`mkCombValid`/
`absValid`/`betaValid`/`eqMpValid`/`deductAntisymValid`/`instValid`/
`instTypeValid`）の `Valid` 保存証明がゼロエラーでコンパイル済み。
4 拡張原理のうち:

- **`new_type_conservative`**（`Soundness.v:2673`）: **無条件**。任意の
  `ModelsTheory F thy` から `ModelsTheory F thy'` を同じ `F` のまま導く
  — 呼び出し側が何も追加で用意する必要がない、文字通り完結した保存性
  証明。
- **`new_constant_conservative`**（`Soundness.v:2737`）: 証明済みだが
  **条件付き**。呼び出し側が新しい定数の意味
  （`fc_n : forall tv ty0, interpType ...`）と、その自然性の証明
  （`fc_n_nat`）を明示的な仮定として渡す必要がある — 「そのような整合的な
  族が任意の到達可能な `F` に対して常に存在する」こと自体は、この補題の
  **外側**の別の事実であり、この補題自体は証明していない（実際、後述の
  master corollary を組み立てる際、この事実は自明ではなく、`Frame0` の
  型演算子解釈が `unit` 定数関数であることに依存する専用の自然性補題
  `interpType_witness_frTyOp0_natural` を新たに証明する必要があった）。

`new_basic_definition_conservative` / `new_basic_type_definition_conservative`
は、証明を試みた末に**意図的に証明せず、その理由を `Soundness.v` 本体に
コメントとして明記する**方針に切り替え、両方ともそのコメントは着地・
コンパイル確認済み（`Soundness.v:2855`/`2876`、§11.3）。

**master corollary は着地済み**だが、当初計画の
`definitional_theories_have_models`（`new_axiom` を除く 4 原理全部を
カバーする想定）とは**別物**として着地させた:
`type_and_constant_theories_have_models`（`Soundness.v`）は
`ReachedFromTypeConst`（`new_type`/`new_constant` のみで到達可能な理論の
関係、`ReachedFrom` という無限定の名前は使っていない）についてのみ
成り立つ。名前・関係名の両方に制限範囲を埋め込んだのは、
`new_basic_definition`/`new_basic_type_definition` を使った理論には
この定理が**何も述べない**ことを、シグネチャを見ただけで読み取れるように
するため。**計画が当初目指した「4 原理全部の保存性」という到達条件は
未達のまま**であり、これはユーザーが承認した縮小ではなく、このセッション
内で実行時に下した判断である（§11.3 末尾も参照）。

副産物として `Frame` レコードの設計欠陥を 1 つ実際に発見・修正した
（§11.2 の (1)）。これは「フィクション上の完全性」ではなく実在するバグで、
修正前の `frConst_subst` の無条件版は **`ModelsTheory` が要求する `NEq`
の等号意味論と両立不可能**だった（この矛盾の証明は `ModelsTheory` の
`NEq` 節と組み合わせて初めて成立する。「`frConst` を全域的に自明な値へ
倒せば無条件版の `frConst_subst` 単体は満たせる」という逆方向の主張は
実際に構築・型検査して確認したものではなく、`interpType` の依存的な
witness/cast の絡みで非自明になり得るため、本分析では立てない — 確立した
のは「両立不可能」の一点のみ）。この発見と修正自体が、この試みの主要な
成果の一つ。

### 11.2 発見された設計上の問題

(1) は無条件フィールドと `ModelsTheory` の要件との**証明された両立不可能性**、
(2)(3) は「この特定の構築の試みが失敗した」という**検証済みの障害**であり、
(2) については「`frTyOp_subst` フィールドそのものが充足不可能である」ことの
証明ではない点に注意（他の構築、あるいは弱めた/理論相対の
`frTyOp_subst` なら成功する可能性は排除していない）。

**(1) `frConst_subst`（修正済み）**: 旧設計は「`frConst F n tv ty` は
*任意の* `n`/`ty` で type-substitution 自然性を満たす」と無条件に要求して
いた。反例: `ty := HTyVar x`、`tyin := [x ↦ mk_fun dom (mk_fun dom bool_ty)]`
とすると、これは `frConst F NEq` に「裸の型変数 `x` において、あらゆる
`dom`/`tv` の選択で同時に、転送された真の等号と一致する」ことを強制する
— `frConst` は不透明な `Type` を覗いて「これは実は等号関係の型か」を
判定できないので、これと `ModelsTheory` の `NEq` = 真の等号という要件を
**同時に**満たす `tv`-全域的な `frConst` は存在しない。
修正: この法則を `Frame` フィールドから外し、`ModelsTheoryNat`
（`Semantics.v`）という理論相対の性質に変えた — `type_match` で
*宣言済み*定数自身の総称型の genuine なインスタンスである `ty` でのみ
要求する（`type_match` は裸の `HTyVar` に対しては構造的に成功し得ないので、
反例の入力自体が排除される）。

**(2) `frTyOp_subst` を使った `new_basic_type_definition` 用の Frame 拡張
構築の試み（未解決、証明としては未完）**: 旧型演算子 `n ≠ tyname` への
委譲（`frTyOp' := frTyOp F`）を軸にした具体的な構築を複数試したが、
いずれも `tv_subst` が `F'`（新しい `tyname` を含む）と `F`（含まない）
とで食い違う場所（`tyin` が何らかの変数を `tyname` を含む型へ写す場合）
で `frTyOp_subst` の無条件版を満たせなかった。(1) と同じ形の
「theory-relative 化」や「`frTyOp` の値は `(n, 既に解釈済みの argsT)`
のみに依存する」という不変条件を試みたが、(3) の witness 欠落と衝突して
いずれも行き詰まった。これは**この特定の構築アプローチの失敗**であり、
`frTyOp_subst` というフィールド自体が原理的に充足不可能であることの
証明ではない — 別の構築（例えば理論相対化そのものをやり直す）が
成功する可能性は本分析では排除していない。

**(3) `frTyOp_inhab` の witness 欠落（未解決、構造的）**: `Frame` の
`frTyOp : (Name -> {T : Type & T}) -> Name -> seq HType -> seq Type -> Type`
は型引数を裸の `seq Type`（witness なし）で受け取るが、
`frTyOp_inhab : forall tv n htyargs args, frTyOp tv n htyargs args`
は**任意の** `args`（`[False]` のような何もない garbage を含む）で
inhabited であることを無条件に要求する。`new_basic_type_definition` が
導入する部分集合型（述語 `pred` に依存する、witness を伴う本物の内容を
持つ型）は、まさにこの witness が必要な場面で `seq Type` からは
witness を復元できない（`interpType_witness_gen` は `HType` と周囲の
`tv` から witness を作るのであって、裸の `Type` からは作れない）。
2 つの独立したサブエージェントのパスで同じ結論に達した。狭い回避策
（`FrTyOpPure` のような追加ハイポセシス）を複数試したが、`tv_local` を
`argsT : seq Type` だけから構築しようとする限りいずれも同じ壁に当たる。
一つの直接的な修正は `frTyOp` の型を `seq Type -> seq {T : Type & T}` に
変える（`args` 自体に witness を持たせる）ことだが、必要性そのものを
証明したわけではない（理論相対の naturality や、モデルをステップごとに
拡張するのではなく最終理論に対して一括構築する方式など、他の repair も
検討したが試していない）。いずれにせよ、これは `Frame`/`interpType`/
`frTyOp_subst`/`frConst_subst`/`ModelsTheoryNat`/`denote` など、既に
検証済みの `Semantics.v`+`Soundness.v`（約 3200 行）の相当部分に波及し
得る変更であり、このセッションの範囲外として着手しなかった。

### 11.3 現在ブロックされているもの・最終状態

- **`new_basic_definition_conservative`**: §11.2 (3) のような構造的問題では
  なく、単に証明が大きい。`ModelsTheoryNat` は定数の生成型に対する
  *任意の*（理論非依存の）`type_match` インスタンスで自然性を要求する
  ため、新しく定義される定数の denotation の自然性を示すには
  「2 つの異なる型代入 `m`/`m'` のもとでの `rhs` の denotation を関係
  づける」新しい JMeq 帰納法（`denote_inst_type` の証明形を一般化した
  もの）が要る。試した 2 つのショートカット（`check_open_term_rematch`
  によるチェック回避、`tv_subst` の合成等式）はいずれもサブエージェントの
  反例チェックで誤りと判明した（`check_open_term` の `TmConst`/`TmAbs`
  枝は代入後の型にも `check_type thy` を要求する（`Kernel.v:150-163`）
  ため前者は不可能、後者は `m = [fmap]` かつ `tyin` が非自明なときに
  反例）。最終的に**証明を完成させず**、`Soundness.v:2855` に理由を
  記したコメントを置く方針に切り替えた。
- **`new_basic_type_definition_conservative`**: §11.2 (3) によりブロック。
  `abs`/`rep` の関数を全域にするには
  `ClassicalEpsilon.excluded_middle_informative`
  （`constructive_indefinite_description` に依存）が必要とも判明した
  — これはこの形式化の `initial_theory` が既に `c_select` を組込み論理
  定数として宣言していることの帰結では**ない**（`initial_theory` が
  宣言する論理定数は `Kernel.v:2159-2162` の通り `NEq` のみ）。この
  公理追加自体は正当化できると判断したが、(3) が別途未解決のため、
  この原理の証明はやはり未完のまま。`Soundness.v:2876` に理由を記した
  コメントを置いた。
- **master corollary**: 上記 2 つを除外した縮小版
  `type_and_constant_theories_have_models`（`ReachedFromTypeConst`
  について）を新規に着地させた（`Soundness.v`）。`Frame0`/
  `initial_theory` 側のモデル構築（`Frame0_models_initial`）と、
  `new_constant` ステップで使う専用の witness 自然性補題
  （`interpType_witness_frTyOp0_natural`、`frTyOp0` が定数 `unit` 関数
  であることに依存する構築）を新たに証明した上で組み立てた。
  **当初計画が目指した、`new_axiom` を除く 4 原理全部をカバーする
  `definitional_theories_have_models` は存在しない** — この点は
  `README.md`/本節双方で明記している。

### 11.4 教訓

- 「`Frame` の各フィールドを無条件の全称命題として書く」設計は、
  実際にそのフィールドが**呼ばれる**入力の範囲（`interpType` 自身の
  再帰が作る、witness 付きで整合した入力だけ）よりも真に強く、
  「定数だけを追加する」拡張（`frame_override` 的な `frConst`-only
  差分）では偶然に無害だが、「本当に新しい中身を持つ型/値」を導入する
  拡張（`new_basic_type_definition` はその最たる例）で初めて牙を剥く。
  同種の設計をレビューする際は、「このフィールドが実際に呼ばれる入力の
  集合」と「このフィールドが要求する入力の集合」が一致しているかを
  最初に確認すべきだった。
- サブエージェントに証明の「ショートカット」を指示する際、指示者
  （このセッションの主エージェント）自身の反例チェックが 2 回連続で
  誤っていた（`check_open_term` の実際の定義を確認せずに
  「`check_type` は不要なはず」と判断した、`tv_subst` の合成等式を
  検証せずに正しいと断定した）。サブエージェント側が反例を実際に
  Rocq で確認して差し戻したことでどちらも実害なく捕捉できたが、
  **証明支援系での「小さな近道」の提案は、提案者自身がその場で
  反例を探すか、最低限 Rocq で試すまでは信用しないこと** — 型検査器の
  ある環境の強みはここにある。同じ理由で、本節自体の草稿にも
  「無条件版 `frConst_subst` は trivial な値で満たせる」という
  未検証の逆方向の主張を一度書いてしまい、外部レビューで指摘されて
  訂正した — **失敗分析を書くときも、確立していない主張を確立した
  ものとして書かないこと**は同じ原則の適用範囲である。
- `new_axiom` に対する計画の既存の哲学（「保存性の証明を要求せず、
  その旨を明記する」）を、今回発覚した `frTyOp_inhab` の構造的な限界
  （`new_basic_type_definition_conservative`）と `new_basic_definition_conservative`
  の未完成に対しても適用した — `Admitted` や偽のハイポセシスで押し通す
  代わりに、`Soundness.v` 本体に理由を明記するコメントを置き、
  master corollary の名前・関係名にも制限範囲を埋め込んだ。ただし
  これは「4 原理全部が保存的」という当初の到達条件を**満たした**という
  意味では全くなく、その条件は明示的に未達のまま残っている。
