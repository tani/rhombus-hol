...
**既知の欠点: `@doc` ブロックが使えていない。** `type` / `function` / `theorem` などは
`#%module_block` が本体を走査して認識しているだけで、束縛ではない。
`@doc` は for-label 束縛を要求するので、これらを索引付きの項目として書けず、
`@verbatim` の文法表示 + `@section` で代用している。検索性が落ちる。

直すなら、宣言ヘッドを `decl.macro` として実際に束縛し（中身は
「rhombus/hol モジュールのトップレベル以外では使えない」というエラー）、
`rhombus/hol` から export する。ドキュメントが `@doc` で書けるようになるうえ、
`block:` の中に宣言を書いたときのエラーメッセージも良くなる。副作用の調査が要るので
未着手。
