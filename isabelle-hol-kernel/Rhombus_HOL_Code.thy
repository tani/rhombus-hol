...
text \<open>@{const check_open_term} asks for @{term "type_of (Comb f x) \<noteq> None"} at every
  application node, and @{const type_of} is itself a full recursive pass, so checking a
  term of size \<open>n\<close> costs \<open>O(n\<^sup>2)\<close> -- and every kernel rule checks the terms it is handed.
  The fused function below checks and computes the type in one bottom-up pass, so the
  same check costs \<open>O(n)\<close>. It is introduced as a code equation only: the specification
  that the soundness development talks about stays @{const check_open_term}.\<close>

fun check_term_type :: "htheory \<Rightarrow> htype list \<Rightarrow> hterm \<Rightarrow> htype option" where
  "check_term_type thy env (FVar _ ty) = (if check_type thy ty then Some ty else None)"
...
next
  case (Const n ty)
  show ?case
    unfolding check_term_type.simps check_open_term.simps
...
text \<open>One memo table, on the fused checker itself, so that a subterm shared between
  successive inference results is checked once. Every other check is a projection of
  it: @{const check_open_term} asks whether it succeeded, @{const check_term} asks the
  same at the empty environment, and @{const check_prop} additionally compares the
  computed type against @{const bool_ty}. The recursive calls go through the memoized
  constant, so the cache is per node rather than per whole term. A cache hit is
  observationally equal to recomputation.\<close>
...
lemma check_term_type_memo_code [code]:
  "check_term_type thy env t = memoize_ternary check_term_type_uncached thy t env"
