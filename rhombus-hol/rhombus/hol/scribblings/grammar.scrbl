...
counterpart of a bracket list literal expression. A nonnegative numeral is an
atomic native @tt{Nat} discrimination. It is compatible with ordinary
@tt{zero()} and @tt{succ(k)} patterns, but never desugars to a successor
chain. A pair pattern (@tt{Pair(x, y)}) matches the language's one product
type.
...
A @tt{String} literal pattern is the ordinary constructor pattern
@tt{text(cons(codepoint, ... nil()))}. Each codepoint uses the atomic native
numeral-pattern rule, so empty, ASCII, and non-ASCII literals have the same
shape complexity per source scalar and obey the normal ordered reachability
