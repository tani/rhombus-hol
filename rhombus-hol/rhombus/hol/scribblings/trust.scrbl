...
@bold{Numerals.} A compact @tt{NatLit(n)} is an Isabelle-verified kernel term
form for the ordinary Peano value @tt{n}; it is neither an object-language
constant nor an axiom. The native Nat extension conservatively installs the
reserved @tt{Nat}, @tt{zero}, and @tt{succ} objects and returns their
zero/successor distinctness, successor injectivity, and induction theorems.

Native numeral conversions are trusted only through their proved Isabelle
implementation and checked theorem output. One conversion exposes
@tt{NatLit(0) = zero()} or
@tt{NatLit(n+1) = succ(NatLit(n))}; equality and arithmetic conversions
produce Boolean equations. The Rhombus façade validates term shape, theory
provenance, and stamps before exposing a @tt{Thm}. Host arithmetic therefore
